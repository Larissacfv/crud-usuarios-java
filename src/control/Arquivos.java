...
public class Arquivos {
 public Arquivos() {//pegar o arquivo de texto e mandar para lá
	 
 }
 
 public void salvaUsuario(Usuario usuario ) throws IOException {
	int id=gerarID();// chamar metodo gerarID
	
	OutputStream os = new FileOutputStream("cadastro.txt",true);//abrir arquivo e true pega e adiciona no final
	BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(os));//prepara para lêr textp
	
	String texto= id+","+                //ID+,+nome+,+idade+,+sexo
	              usuario.getNome()+","+  
			      usuario.getIdade()+","+
	              usuario.getSexo();
	 bw.write(texto);
	 ...
	 bw.close();
        ...
        public void reescreverArquivo(ArrayList<String> linhas) throws IOException {
        	OutputStream os =new FileOutputStream("cadastro.txt");//apaga todo o conteúdo antigo e começa do zero é o que quero no atualizar.
            OutputStreamWriter osw = new OutputStreamWriter(os);
        	BufferedWriter bw = new BufferedWriter(osw);
        	
        	for(int i =0; i< linhas.size(); i++) { //pega cada linha, reescreve o arquivo deixando a versão atualizada
		...
		public void cadastrarUsuario(String nome, int idade, char sexo) throws IOException {
			...
			if(!linhas.isEmpty()) { //"Se a lista NÃO estiver vazia"
				...
				String ultimaLinha= linhas.get(linhas.size()-1);//linhas.size() → quantidade de elementos-1 → porque índice começa em 0
